<?php  
  ...
  if (isset($_POST['team_join']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    global $errors;
    $checkArrays = [
        'team_name' => 'Team Name' , 
  ...
  $teamRequested = selectAllOneCol("published_teams" , "team_join_pass" , array(
    'team_name' => $_POST['team_name'] 
  ) );
  if (empty($teamRequested)) {
    array_push($errors , "Invalid Credentails");
  }
  if (isset($_POST['team_join_pass']) && $teamRequested ) {
    if (  !( sha1($_POST['team_join_pass']) == $teamRequested[0]['team_join_pass'] ) ) {
   ...
   if (!empty($checkAlready)) {
    array_push($errors , "You Already Joined That Team");
   }
  
   if (count($errors) == 0) {
    global $conn;
  
    unset($_POST['team_join']);
...
//     $_POST['body_post'] = htmlentities($_POST['body_post']);
        ...
        $_SESSION['joined_team_id'] =$joinId;
 ...
 // header("location: " . BASE_URL . "/index.php"); 
  ...
  ?>
        ...
        <section class="heroarea__main--div" style="background: linear-gradient(rgba(0,0,0,0.2) , rgba(0,0,0,0.3)) , url('./images/heroarea/literary-05.jpg') center / cover no-repeat;" >
            <div class="flex-col">
                <h1 class="headline--main-bg"> صفحة الانضمام لفريق</h1>
            </div>
        </section>
       <!-----------------END HEROAREA-------------------->
       ...
       <div class="mt-5"></div>
                ...
                <?php  include  ROOT_PATH_MAIN . '/includes/templates/formErrors.php'  ?>
                        ...
                        <div class="singupform__div">
                            <div class="d-grid grid-signupform">
                                ...
                                <div class="g-col-2">
                                 ...
                                 <h1 class="headline--main-2">   الانضمام لفريق </h1>
                               
                                 <p class="mian--paragraph-sm mt-1">تريد الانشاء بدلا من لك   ؟  <span class="gothought_a"> اضغط هنا </span>   </p>
                  ...
                  <input type="submit" name="team_join" value="الانضمام لفريق" class="singup_btn" >
                                 </form>
                                    </div>
